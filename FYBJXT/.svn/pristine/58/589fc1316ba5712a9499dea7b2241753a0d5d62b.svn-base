package com.xyw.sys.custom.component
{
	...
	public class CustomCheckBoxTree extends Tree
	{
		protected var _checkField: String;
		private var _checkFunction: Function;
		
		public function CustomCheckBoxTree()
		{
			super( );
			this.itemRenderer = new ClassFactory(CustomCheckBoxTreeItemRender );
		...
		{
			var value: String;
			var state: int = ( event.itemRenderer as CustomCheckBoxTreeItemRender ).checkBox.checkState;
		...
		{
			var treeListData: TreeListData = new CustomCheckBoxTreeListData(itemToLabel( data ), uid, this, rowNum );
		...
		override protected function initListData( item: Object, treeListData: TreeListData ): void
		{
			...
			( treeListData as CustomCheckBoxTreeListData ).checkedState = itemToCheck( item );
		}	    
